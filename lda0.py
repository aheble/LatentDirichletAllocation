...
def LDA(A, B, K, alpha, gamma):
    ...
    swd2 = sparse((B[:, 2], (B[:, 1]-1, B[:, 0]-1))).tocsr()
    ...
    entropy = np.zeros((num_gibbs_iters, K))
    for itera in range(num_gibbs_iters):
        ...
        for m in range(K):
            ...
            sbeta = swk[:,m]
            for word in range(W):
                beta=(gamma+sbeta[word])/((gamma*W)+np.sum(sbeta))
                entropy[itera,m]-=beta*np.log(beta)
    ...
    return perplexity, swk, mix_prop, entropy
...
if __name__ == '__main__':
    ...
    perplexity, swk, mix_prop, entropy = LDA(A, B, K, alpha, gamma)
    ...
    plt.xlim(left=1)
    plt.title("Topic Posteriors Against Gibbs Sampling Iteration")
    plt.show()
    
    plt.figure(figsize=[10,5])
    for i in range(K):
        plt.plot(xaxis, entropy[:,i])
    plt.ylabel("Entropy")
    plt.xlabel("Iteration")
    plt.xlim(left=1)
    plt.title("Word Entropy Against Gibbs Sampling Iteration")
    plt.show()
