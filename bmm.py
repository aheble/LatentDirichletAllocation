...
def BMM(A, B, K, alpha, gamma):
    ...
    num_iters_gibbs = 50
    mix_prop = np.zeros((num_iters_gibbs, K))
    # Perform Gibbs sampling through all documents and words
    for itera in range(num_iters_gibbs):
        for d in range(D):
        ...
        #for m in range(K):
            #theta_k = (alpha+sk_docs[m])/((alpha*K)+D)
    ...
    return perplexity, swk, mix_prop
...
if __name__ == '__main__':
    ...
    perplexity, swk, mix_prop = BMM(A, B, K, alpha, gamma)
